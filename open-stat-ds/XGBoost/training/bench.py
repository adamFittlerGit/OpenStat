...
df = df.drop(columns=['Event', 'Age_Class', 'Weight_Class', 'Squat', 'Deadlift', 'Total', 'Place', 'Dots', 'Tested', 'Country', 'Federation'])
...
df = df.groupby('Name').apply(lambda x: pd.Series({
    ...
    'Bench_1': x['Bench'].iloc[-3] if len(x) >= 3 else None,
    'Bench_2': x['Bench'].iloc[-2] if len(x) >= 2 else None,
    'Bench_3': x['Bench'].iloc[-1] if len(x) >= 1 else None
})).reset_index()
...
df = df[(df['Bench_1'] != 0.0) & (df['Bench_2'] != 0.0) & (df['Bench_3'] != 0.0) & (df['Bench_1'] <= df['Bench_2']) & (df['Bench_2'] <= df['Bench_3'])]
print(df.shape)
...
print("---- Saving Model ----\n")
# ---- Save the Model ---- 
joblib.dump(opt.best_estimator_, '../models/xgb_model_bench.pkl')
joblib.dump(encoder, '../encoders/encoder_bench.pkl')
joblib.dump(scaler, '../scalers/scaler_bench.pkl')

# Save the printed variables in model_info.txt
with open('../info/stats/model_info_bench.txt', 'w') as f:
...
plt.savefig('../info/graphs/bench/feature_importance.png')
plt.close()
...
with open('../info/stats/model_info_bench.txt', 'a') as f:
...
plt.savefig('../info/graphs/bench/actual_vs_predicted.png')
plt.close()
...
plt.savefig('../info/graphs/bench/residual_plot.png')
plt.close()
...
plt.savefig('../info/graphs/bench/error_distribution.png')
plt.close()
