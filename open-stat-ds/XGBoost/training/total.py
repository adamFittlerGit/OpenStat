...
df = df.drop(columns=['Event', 'Age_Class', 'Weight_Class', 'Bench', 'Deadlift', 'Squat', 'Place', 'Dots', 'Tested', 'Country', 'Federation'])
...
df = df.groupby('Name').apply(lambda x: pd.Series({
    ...
    'Total_1': x['Total'].iloc[-3] if len(x) >= 3 else None,
    'Total_2': x['Total'].iloc[-2] if len(x) >= 2 else None,
    'Total_3': x['Total'].iloc[-1] if len(x) >= 1 else None
})).reset_index()
...
print("---- Saving Model ----\n")
# ---- Save the Model ---- 
joblib.dump(opt.best_estimator_, '../models/xgb_model_total.pkl')
joblib.dump(encoder, '../encoders/encoder_total.pkl')
joblib.dump(scaler, '../scalers/scaler_total.pkl')

# Save the printed variables in model_info.txt
with open('../info/stats/model_info_total.txt', 'w') as f:
...
plt.savefig('../info/graphs/total/feature_importance.png')
plt.close()
...
with open('../info/stats/model_info_total.txt', 'a') as f:
...
plt.savefig('../info/graphs/total/actual_vs_predicted.png')
plt.close()
...
plt.savefig('../info/graphs/total/residual_plot.png')
plt.close()
...
plt.savefig('../info/graphs/total/error_distribution.png')
plt.close()
