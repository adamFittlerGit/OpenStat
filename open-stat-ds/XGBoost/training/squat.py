...
df = df.drop(columns=['Event', 'Age_Class', 'Weight_Class', 'Bench', 'Deadlift', 'Total', 'Place', 'Dots', 'Tested', 'Country', 'Federation'])
...
df = df.groupby('Name').apply(lambda x: pd.Series({
    ...
    'Squat_1': x['Squat'].iloc[-3] if len(x) >= 3 else None,
    'Squat_2': x['Squat'].iloc[-2] if len(x) >= 2 else None,
    'Squat_3': x['Squat'].iloc[-1] if len(x) >= 1 else None
})).reset_index()
...
plt.savefig('../info/graphs/squat/feature_importance.png')
plt.close()
...
with open('../info/stats/model_info_squat.txt', 'a') as f:
...
plt.savefig('../info/graphs/squat/actual_vs_predicted.png')
plt.close()
...
plt.savefig('../info/graphs/squat/residual_plot.png')
plt.close()
...
plt.savefig('../info/graphs/squat/error_distribution.png')
plt.close()
